...
function formatDate(date) {
  const newDate = date.replace(/\.000Z/, '').split('T');
  ...
  newDate[0] = newDate[0]
    ...
    .map((el) => el.replace(/^0/, ''))
    .join('/');
  newDate[1] = newDate[1].replace(
    /^(\d{2})([:\d]+)/,
    function callBack(match0, match1, match2) {
      let a = match1;
      if (+match1 > 12) {
        a = `${+a - 12 + match2} PM`;
      } else if (+a < 12) {
        a = `${a.replace(/(0)(\d)/, '$2') + match2} AM`;
      } else {
        a = `${a + match2} PM`;
      }
      return a;
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 365 * 100; i += 1) {
    currentDate.setDate(currentDate.getDate() + 1);
    if (currentDate.getDate() === 13 && currentDate.getDay() === 5) {
      return currentDate;
    }
  }
  throw new Error(`Couldn't find next Friday the 13th`);
}
...
function getQuarter(date) {
  ...
  let quarter;
  if (quarters['1'].includes(currentMonth)) {
    quarter = 1;
  }
  if (quarters['2'].includes(currentMonth)) {
    quarter = 2;
  }
  if (quarters['3'].includes(currentMonth)) {
    quarter = 3;
  }
  if (quarters['4'].includes(currentMonth)) {
    quarter = 4;
  }
  return quarter;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      result.push(formatGetDate(currentDate));
      currentDate.setDate(currentDate.getDate() + 1);
    }
